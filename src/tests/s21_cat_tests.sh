...
do
    ...
    do
        ...
        if [ -s temp_result ]; then
            echo "./s21_cat $options ../tests/samples/text_$number FAIL" >> ../tests/log; fail=$(( $fail + 1 ))
        else
            echo "./s21_cat $options ../tests/samples/text_$number SUCCESS" >> ../tests/log; success=$(( $success + 1 ))
        fi
    done
done
